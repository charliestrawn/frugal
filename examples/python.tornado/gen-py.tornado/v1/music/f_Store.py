...
class _buyAlbum(FProcessorFunction):
    ...
    def process(self, ctx, iprot, oprot):
        ...
        try:
            ...
            with (yield self._lock.acquire()):
                _write_application_exception(ctx, oprot, "buyAlbum", ex_code=TApplicationExceptionType.INTERNAL_ERROR, message="Internal error processing {}: {}".format("buyAlbum", e.message))
...
class _enterAlbumGiveaway(FProcessorFunction):
    ...
    def process(self, ctx, iprot, oprot):
        ...
        try:
            ...
            exc_info = sys.exc_info()
            with (yield self._lock.acquire()):
                _write_application_exception(ctx, oprot, "enterAlbumGiveaway", ex_code=TApplicationExceptionType.INTERNAL_ERROR, message="Internal error processing {}: {}".format("enterAlbumGiveaway", e.message))
            raise exc_info[0], exc_info[1], exc_info[2]
        with (yield self._lock.acquire()):
            try:
                oprot.write_response_headers(ctx)
