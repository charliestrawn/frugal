...
func (g *Generator) nullsafe() bool {
	_, ok := g.BaseGenerator.Options[nullsafe]
	return ok
}
...
func (g *Generator) GenerateDocStringComment(file *os.File) error {
	comment := fmt.Sprintf(
		"// Autogenerated by Frugal Compiler (nullsafe) (%s)\n"+
...
func (g *Generator) generateConstantValue(t *parser.Type, value interface{}, ind string, asConst bool) (result string, isConst bool) {
	...
	if underlyingType.IsPrimitive() || underlyingType.IsContainer() {
		...
		case "binary":
			return fmt.Sprintf("Uint8List.fromList(utf8.encode('%s'))", value), false
		case "list", "set":
			contents := ""
			var valuesConst bool
			if underlyingType.Name == "set" {
				contents += fmt.Sprintf("Set<%s>.from(",
		...
		contents += fmt.Sprintf("%s()", dartType)
		for _, pair := range value.([]parser.KeyValue) {
...
func (g *Generator) generateEnumUsingClasses(enum *parser.Enum) string {
	...
	contents += tab + "static final Set<int> VALID_VALUES = Set.from([\n"
	for _, field := range enum.Values {
...
func (g *Generator) generateStruct(s *parser.Struct, kind structKind) string {
	...
	for _, field := range s.Fields {
		...
		contents += fmt.Sprintf(tab+"%s %s%s;\n",
			g.getDartTypeFromThriftType(field.Type)+nullable, fieldName, g.generateInitValue(field, kind))
		if g.shouldGenerateFieldId(s, kind) {
...
func (g *Generator) generateWriteFieldRec(field *parser.Field, first bool, ind string) string {
	...
	if underlyingType.IsPrimitive() {
		...
		case "map":
			...
			keyElem := g.GetElem()
			keyField := parser.FieldFromType(underlyingType.KeyType, keyElem)
			valField := parser.FieldFromType(underlyingType.ValueType, fmt.Sprintf("%s%s[%s]", fName, bang, keyElem))
...
func (g *Generator) generateClone(s *parser.Struct) string {
	...
	if len(s.Fields) == 0 {
		...
		contents += fmt.Sprintf(tabtab+"return %s();\n", s.Name)
	...
	for _, field := range s.Fields {
		...
		contents += fmt.Sprintf(tabtab+"%s %s,\n", g.getDartTypeFromThriftType(field.Type)+nullable, fieldName)

	}
	contents += tab + "}) {\n"

	contents += fmt.Sprintf(tabtab+"return %s()", s.Name)
	for _, field := range s.Fields {
		fieldName := toFieldName(field.Name)
		contents += "\n"
		contents += ignoreDeprecationWarningIfNeeded(tabtabtab, field.Annotations)
...
func (g *Generator) GeneratePublisher(file *os.File, scope *parser.Scope) error {
	...
	publishers += fmt.Sprintf("%s %sFactory(frugal.FScopeProvider provider, {List<frugal.Middleware>%s middleware}) =>\n",
		publisherClassname, lowercaseFirstCharacter(publisherClassname), nullable)
...
func (g *Generator) GenerateSubscriber(file *os.File, scope *parser.Scope) error {
	...
	subscribers += fmt.Sprintf("%s %sFactory(frugal.FScopeProvider provider, {List<frugal.Middleware>%s middleware}) =>\n",
		subscriberClassname, lowercaseFirstCharacter(subscriberClassname), nullable)
	...
	for _, op := range scope.Operations {
		...
		subscribers += tabtab + "return frugal.FSubscription(topic, transport);\n"
		subscribers += tab + "}\n\n"
		...
		subscribers += fmt.Sprintf(tabtab+"frugal.FMethod method = frugal.FMethod(on%s, '%s', 'subscribe%s', this._middleware);\n",
...
func (g *Generator) generateClient(service *parser.Service) string {
	...
	contents += fmt.Sprintf("%s %sFactory(frugal.FServiceProvider provider, {List<frugal.Middleware>%s middleware}) =>\n",
		clientClassname, lowercaseFirstCharacter(clientClassname), nullable)
...
func (g *Generator) generateReturnArg(method *parser.Method) string {
	...
	return fmt.Sprintf("<%s>", g.getDartTypeFromThriftType(method.ReturnType)+nullable)
}
