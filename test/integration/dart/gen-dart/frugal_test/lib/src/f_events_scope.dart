...
EventsPublisher eventsPublisherFactory(frugal.FScopeProvider provider,
...
class EventsPublisher {
  ...
  EventsPublisher(frugal.FScopeProvider provider,
    ...
    this._methods = {};
    this._methods['EventCreated'] = frugal.FMethod(
        this._publishEventCreated, 'Events', 'publishEventCreated', combined);
  }
  ...
  Future publishEventCreated(frugal.FContext ctx, String preamble,
      String ramble, String role, String RID, t_frugal_test.Event req) {
    return this
        ._methods['EventCreated']([ctx, preamble, ramble, role, RID, req]);
  }

  Future _publishEventCreated(frugal.FContext ctx, String preamble,
      String ramble, String role, String RID, t_frugal_test.Event req) async {
...
EventsSubscriber eventsSubscriberFactory(frugal.FScopeProvider provider,
        {List<frugal.Middleware> middleware}) =>
...
class EventsSubscriber {
  ...
  Future<frugal.FSubscription> subscribeEventCreated(
      String preamble,
      String ramble,
      String role,
      String RID,
      dynamic onEvent(frugal.FContext ctx, t_frugal_test.Event req)) async {
    var op = 'EventCreated';
    var prefix = '$preamble.$ramble.$role.$RID.';
    var topic = '${prefix}Events$delimiter$op';
    var transport = provider.subscriberTransportFactory.getTransport();
    await transport.subscribe(
        topic, _recvEventCreated(op, provider.protocolFactory, onEvent));
    return frugal.FSubscription(topic, transport);
  }

  frugal.FAsyncCallback _recvEventCreated(
      String op,
      frugal.FProtocolFactory protocolFactory,
      dynamic onEvent(frugal.FContext ctx, t_frugal_test.Event req)) {
    frugal.FMethod method =
        frugal.FMethod(onEvent, 'Events', 'subscribeEvent', this._middleware);
